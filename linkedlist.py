...
class LinkedList(object):
    ...
    def length(self):
        ...
        Best case runtime: O(1) where we return self.size. Handled in adding and deleting nodes
        Worst case runtime: O(n) where n is the number of nodes in the linkedlist because we need to loop through every node
        """
    ...
    def insert_at_index(self, index, item):
        ...
        if index == 0:
            ...
            # new node's next pointer goes to curr node
            iterations = 0
            prev = None
            curr = self.head
            while curr is not None:
                if iterations == index:
                    prev = curr             # prev is the node that is before new_node once we insert it
                    curr = curr.next        # curr is the node after new_node once we insert it
                    prev.next = new_node
                    new_node.next = curr
                else:                       # loop to the next iteration and next nodes
                    prev = curr
                    curr = curr.next
                    iterations += 1
            self.size += 1
